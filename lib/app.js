...
app.delete('/posters/:id', async(req, res) => {
  ...
  const data = await client.query('DELETE FROM posters AS p WHERE p.id=$1;', [posterId]);
...
app.post('/posters', async(req, res) => {
  try {
    ...
    const data = await client.query(`
    INSERT INTO posters(name, description, in_stock,   price, owner_id, categories_id)
    VALUES($1, $2, $3, $4, $5, $6)
    RETURNING *
    `, [realNewPoster.name, realNewPoster.description, realNewPoster.in_stock, realNewPoster.price, fakeUser.id, realNewPoster.categories_id]); 
    
    console.log(data, 'weeeeeeooooooooooooooooo');
    res.json(data);
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
